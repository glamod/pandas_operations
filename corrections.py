...
def gen_files(data, dataset, correction_path, yr, mo):
    prepend = dataset.split("_")[0]+"-"+dataset.split("_")[1][1:6].replace(".",
                                                                           "")+"-"
    for f in ["duplicates", "duplicate_flags", "id", "longitude", "latitude",
              "timestamp"]:
        os.makedirs(os.path.join(correction_path, f), exist_ok=True)    
    df = pd.DataFrame({"UID": data[("c98", "UID")], 
                       "ID": data[("core", "ID")],
                       "LON": data[("core", "LON")],
                       "LAT": data[("core", "LAT")]})
    ...
    minutes = np.round(
        60*np.fmod(data[("core","HR")], 1)).astype("Int64")    
    df["TM"] = pd.to_datetime(
        pd.DataFrame({"YR" : int(yr)*np.ones(hours.shape, dtype=int), 
                      "MO" : int(mo)*np.ones(hours.shape, dtype=int),
                      "DY" : data[("core","DY")].astype("Int64"),
                      "H" : hours, "M" : minutes}).astype(str).apply(
                          "-".join, axis=1).values, 
    ...
    #df['UID'] = df['UID'].apply(lambda x: f"{prepend+x}")    
    #%% duplicates
    fn = os.path.join(correction_path, "duplicates", 
                      yr+"-"+mo+".txt.gz")
    fn_f = os.path.join(correction_path, "duplicate_flags", 
                      yr+"-"+mo+".txt.gz")
    if df.empty == False:
        dup, dup_f = get_dup(data, dataset)
        if os.path.exists(fn):
            print(fn)
            df1 = pd.read_csv(fn, delimiter = '|', dtype = 'object',
                              header = None, usecols=[0, 1, 2], 
                              names = ["UID", "UID_d", "flag"], quotechar=None,
                              quoting=3)
            os.remove(fn) 
            pd.concat(
                [df1.astype(str), dup.astype(str)]).drop_duplicates(
                    subset ="UID", keep="last").to_csv(
    ...
    if df.empty == False:
        ...
        if os.path.exists(fn):
            df1 = pd.read_csv(fn, delimiter = '|', dtype = 'object',
                              header = None, usecols=[0, 1, 2], 
                              names = ["UID", "ID", "flag"], quotechar=None,
                              quoting=3)
            os.remove(fn)
            pd.concat([df1.astype(str), 
                      df[["UID", "ID", "flag"]].astype(str)]).drop_duplicates(
                          subset ="UID", keep="last").to_csv(
                              fn, sep="|", header=False, index=False, 
                              compression='infer')
        else:
            df[["UID", "ID", "flag"]].astype(str).to_csv(
                fn, sep="|", header=False, index=False, compression='infer')
    #%% longitude 
    fn = os.path.join(correction_path, "longitude", 
                      yr+"-"+mo+".txt.gz")
    if df.empty == False:
        if os.path.exists(fn):
            df1 = pd.read_csv(fn, delimiter = '|', dtype = 'object',
                              header = None, usecols=[0, 1, 2], 
                              names = ["UID", "LON", "flag"], quotechar=None,
                              quoting=3)
            os.remove(fn)
            pd.concat([df1.astype(str), 
                      df[["UID", "LON", "flag"]].astype(str)]).drop_duplicates(
                          subset ="UID", keep="last").to_csv(
                              fn, sep="|", header=False, index=False, 
                              compression='infer')
        else:
            df[["UID", "LON", "flag"]].astype(str).to_csv(
                fn, sep="|", header=False, index=False, compression='infer')
    # latitude
    fn = os.path.join(correction_path, "latitude", 
                      yr+"-"+mo+".txt.gz")
    if df.empty == False:
        if os.path.exists(fn):
            df1 = pd.read_csv(fn, delimiter = '|', dtype = 'object',
                              header = None, usecols=[0, 1, 2], 
                              names = ["UID", "LAT", "flag"], quotechar=None,
                              quoting=3)
            os.remove(fn)
            pd.concat([df1.astype(str), 
                      df[["UID", "LAT", "flag"]].astype(str)]).drop_duplicates(
                          subset ="UID", keep="last").to_csv(
                              fn, sep="|", header=False, index=False, 
                              compression='infer')
        else:
            df[["UID", "LAT", "flag"]].astype(str).to_csv(
                fn, sep="|", header=False, index=False, compression='infer')
    # timestamp
    fn = os.path.join(correction_path, "timestamp", 
                      yr+"-"+mo+".txt.gz")
    if df.empty == False:
        if os.path.exists(fn):
            df1 = pd.read_csv(fn, delimiter = '|', dtype = 'object',
                              header = None, usecols=[0, 1, 2], 
                              names = ["UID", "TM", "flag"], quotechar=None,
                              quoting=3)
            os.remove(fn)
            pd.concat(
                [df1.astype(str),
                 df[["UID", "TM", "flag"]].astype(str)]).drop_duplicates(
                    subset ="UID", keep="last").to_csv(
                        fn, sep="|", header=False, index=False, 
                        compression='infer')
        else:
...
def corrections(data, dataset, correction_path, yr, mo):
    ...
    if isinstance(data, pd.DataFrame):
        ...
        buffer = StringIO()
        data_copy = TextParser_hdlr.make_copy(data)
        for dt in data_copy:
            gen_files(dt, dataset, correction_path, yr, mo)
        buffer.seek(0)
        data_copy.close()
        data = TextParser_hdlr.restore(data)
        return
#%%
def split_list(n):
...
def get_dup(data, dataset):
    ...
    tmp_id = tmp_id.assign(distance=[*map(levenshtein.distance, 
                                          tmp_id.ID, tmp_id.ID_s)])
    tmp_uid = pd.DataFrame()
    tmp_uid["UID"] = df_dup["UID"].copy()
    tmp_uid["UID_s"] = df_dup["UID"].shift().astype(str)
    tmp_uid = tmp_uid.assign(distance=[*map(levenshtein.distance, 
                                            tmp_uid.UID, tmp_uid.UID_s)])
    loc = (abs(df_dup['LON'] - df_dup['LON'].shift()) <= tol["LON"]) & \
    ...
    dup_flag = pd.DataFrame({"UID": df["UID"].copy(), "dup_flag": 0,
                             "flag": 1})
    dup_flag["dup_flag"][loc] = 1
    #   %%
    dup_list = df_dup.sort_values(
        by=["LON", "LAT", "DY", "HR"]).loc[df_dup["flag"] == 1].index.to_list()    
    ...
    for index in ind:
        ...
        for fe  in nlst:
            ...
            dup_out = dup_out.append({"UID":df["UID"].loc[fe], "UID_d": tmp,
                                            "flag":  int(1)}, ignore_index=True)
