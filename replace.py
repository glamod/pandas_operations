...
def replace_columns(df_l, df_r, pivot_c = None, pivot_l = None,
                   pivot_r = None, rep_c = None, rep_map = None, log_level = 'INFO'):
    ...
    if not pivot_c and not (pivot_l and pivot_r):
        ...
        pivot_l = pivot_c;pivot_r = pivot_c    
    # Now index on pivot
    df_l.set_index(pivot_l,inplace = True, drop = False)
    df_r.set_index(pivot_r,inplace = True, drop = False)
        
    if not rep_c and not rep_map:
        logger.error('Replacement columns must be declared with a list (rep_c) or a dictionary (rep_map)')
        return 
